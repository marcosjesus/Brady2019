...
begin
  ...
  if FDQueryAux.IsEmpty then
     raise Exception.Create('Planilha L210 não gerada para o período informado.');
            ...
            if FDQueryGridCODIGO_LINHA.AsString = '29B' then
             begin
                ...
                begin
                  ...
                  Style.Brush.BackgroundColor := varCor1;
                  AsVariant :=  'VARIAÇÃO DE CUSTO MENSAL'
             ...
             begin
                ...
                begin
                  ...
                  Style.Brush.BackgroundColor := varCor1;
                  AsVariant :=  'CUSTO DOS PRODUTOS DE FABRICAÇÃO PRÓPRIA VENDIDOS'
...
begin
  ...
  begin
    if FDQueryGrid.IsEmpty then
      raise Exception.Create('Não há dados para serem exportados.');
...
begin
  ...
  if cxAno.Text = EmptyStr then
    raise Exception.Create('Ano não informado');
  ...
  begin
    If  MessageDlg('Processo deste ano já foi executado.Deseja reimportar o ano de ' + cxAno.Text +' novamente?',mtConfirmation,[mbyes,mbno],0)=mryes
    then
     begin
...
begin
  ...
  CriarSaldo(IntToStr(Ano_Ant), IntToStr(Ano), '29A','Variação do Custo Mensal','11');
...
begin
  ...
  CriarSaldo(IntToStr(Ano_Ant), IntToStr(Ano),  '31','CUSTO DOS PRODUTOS DE FABRICAÇÃO PRÓPRIA VENDIDOS', '14');
...
begin
  if cxAno.Text = EmptyStr then
    raise Exception.Create('Período não informado');
  ...
  begin
    ...
    FDQueryGrid.Open;
    if FDQueryGrid.IsEmpty then
       MessageDlg('Ano selecionado não importado do SAP',mtInformation,[mbOk],0);
