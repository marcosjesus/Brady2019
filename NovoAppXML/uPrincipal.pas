...
type
  TfrmPrincipal = class(TForm)
    ...
    dxnImportarSAP: TdxNavBarItem;
    ActConsultaSAP: TAction;
    dxnCadastroQuerySAP: TdxNavBarItem;
    ...
    procedure ActConsultaSAPExecute(Sender: TObject);
  private
...
uses Global, Login,  DBConect, uConsultaDocumento, uUsuario, uSped,Parametros,
      ...
      uContaSaldo,UProcessarL210,uImportarSAP,uCadConsultasSAP;
...
procedure TfrmPrincipal.ActConsultaSAPExecute(Sender: TObject);
begin
    If ( Not Glob_GetAccess('CAD_CONSULTA_SAP','') ) Then
   Begin
      Mens_MensInf('Operação ( CAD_CONSULTA_SAP ) não liberada para o usuário !') ;
      Exit;
   End ;

   frmCadConsultasSAP:=TfrmCadConsultasSAP.Create(Self);
   frmCadConsultasSAP.Show;
end;
