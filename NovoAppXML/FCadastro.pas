...
type
  ...
  public
    ...
    Procedure DesabilitaIDTY ; Virtual;
    Procedure HabilitaIDTY ; Virtual;
    ...
    Procedure Imprime ; Virtual ;
		...
		Function  VerChave(wCampo, wTabela, wValor : String;wTipoDados:Integer):Boolean ;
...
begin
  ...
  try
    ...
    begin
      ...
      Connection := DB_Conect.FDConnection ;              // Nome do database corrente
...
procedure TFrmCadastro.DesabilitaIDTY;
begin
   if VarIDENTITY then
     Tab.ExecSql('SET IDENTITY_INSERT ' + FormTabela + ' ON');
...
Function TFrmCadastro.VerChave(wCampo, wTabela, wValor : String;wTipoDados:Integer):Boolean;
var
	Tab:TFDQuery;
begin
	if VarOperacao <> 1 then
	...
	try
		...
		begin
			...
			Connection := DB_Conect.FDConnection ;
			SQL.Clear;
			SQL.Add('Select '+wCampo+' from '+wTabela+' where '+wCampo+' = :P0 ');
			if wTipoDados = 1 then
				Params[0].AsInteger:=StrToInt(wValor);
			if wTipoDados = 2 then
				Params[0].AsString:=wValor;
			OPen;
...
procedure TFrmCadastro.HabilitaIDTY;
begin
   Tab.ExecSql('SET IDENTITY_INSERT ' + FormTabela + ' OFF');
end;
...
//  try
    try
      ...
      TmpTab.Connection := DB_Conect.FDConnection ;              // Nome do database corrente
