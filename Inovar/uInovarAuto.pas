...
uses uConexao, uCliente, uGerarXML, uImportaNF;
...
begin
  // Assuming here the image will be clicked
  SelectedGroup := TPanel(TControl(Sender).Parent).Name;
  if not Assigned(ImportaNF) then
    ImportaNF := TImportaNF.Create(Self);
  ImportaNF.Show;
  ImportaNF.BringToFront;
end;
...
begin
  // Assuming here the image will be clicked
  SelectedGroup := TPanel(TControl(Sender).Parent).Name;
  if not Assigned(GerarXMLForm) then
    GerarXMLForm := TGerarXMLForm.Create(Self);
  GerarXMLForm.Show;
  GerarXMLForm.BringToFront;
end;
