...
begin
 ...
 if ALote
  then begin
   ...
   if AProvedor in [proAbaco, proIssCuritiba, proFISSLex, proFiorilli]
    then URI := '';
...
//                                    '') +
  ...
  //      raise Exception.Create('Não encontrei inicio do URI: Id=');
     ...
     // Incluido por Italo em 15/09/2015 
     if AProvedor in [proAbaco, proIssCuritiba, proFISSLex, proFiorilli]
       then URI := '';
   ...
   if ALote
    then begin
     ...
     if (URI = '') or (AProvedor in [proRecife, proRJ, proAbaco, proIssDSF, proInfisc,
                                     ...
                                     proPublica, proPronim, proCTA, proEReceita])
      then AID := '>'
//      else if AProvedor = proNatal then AID := ' ' + Identificador + '="Ass_lote">'
      else AID := ' ' + Identificador + '="AssLote_' + URI + '">';

     if AProvedor in [proAbaco, proIssCuritiba, proFISSLex, proPublica, proFiorilli]
      then URI := '';
...
//                                       '') +
    ...
    //      raise Exception.Create('Não encontrei inicio do URI: Id=');
       ...
       if (URI = '') or (AProvedor in [proActcon, profintelISS, proRecife, proNatal,
             proRJ, proGovBR, proPronim, proTecnos, proPublica, proTinus, proEReceita])
        then AID := '>'
        else AID := ' ' + Identificador + '="Ass_' + URI + '">';
...
//                                         '') +
   ...
   if ALote
    then begin
     if (AProvedor in [proEquiplano, proPronim, proIssDSF, proInfisc, proCTA, proEReceita])
...
//                                     '') +
  ...
  //Não é possível assinar por delphi quando o id é minúsculo... mas se manter o id maiúsculo
  //não será validado o schema... uma função mais legível como stringreplace faz a assinatura sumir.
  if Aprovedor in [proPronim, proPublica, proLexsom, proSalvador, proEReceita] then
  begin
    I := pos('Id=', AXMLAssinado);
    if i>0 then
      AXMLAssinado[I] := 'i';
...
begin
 ...
 if DFeUtil.EstaVazio(APathSchemas)
  then schema_filename := PChar(PathWithDelim(ExtractFileDir(DFeUtil.ExeName)) + 'Schemas\' + Servico)
  else schema_filename := PChar(PathWithDelim(APathSchemas) + Servico);
