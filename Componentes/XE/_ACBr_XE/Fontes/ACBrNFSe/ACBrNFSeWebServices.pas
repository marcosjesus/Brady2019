...
uses
  ...
  {$ENDIF}
    ...
    ACBrProvedorCTA, ACBrProvedorEReceita;
...
begin
 ...
 if not (FProvedor in [proGovBr, proSimplISS, proAbaco, proISSNet, pro4R, proIssDSF, proInfisc,
                       ...
                       proPVH, proFreire, proTecnos, proPronim, proPublica, proEReceita,
                       proEgoverneISS, proActcon, proDBSeller, proLexsom, proCTA])
                       and (FConfiguracoes.WebServices.CodigoMunicipio <> 2307650) //Maracanaú
                       and (FConfiguracoes.WebServices.CodigoMunicipio <> 2304400) //Fortaleza
  then
  begin
...
begin
 ...
 case FProvedor of
  ...
  proEReceita:    FProvedorClass := TProvedorEReceita.Create;
 end;
  ...
  then begin
   ...
   begin
     case FProvedor of
      ...
      proAgili, proVirtual, proFreire, proLink3, proEReceita,
     ...
     begin
       ...
       begin
         with TNFSeConsultarNfseRPS(Self).FNotasFiscais.Items[I] do
           ...
           begin
             Gerador.wGrupoNFSe('RPS Id="rps:' + OnlyNumber(NFSe.IdentificacaoRps.Numero) + '"');
   ...
   for i := 0 to TNFSeGerarNFSe(Self).FNotasFiscais.Count-1 do
    begin
     case FProvedor of
      ...
      proEReceita: vNotas := vNotas +
                              '<' + Prefixo4 + 'Rps xmlns="http://www.abrasf.org.br/nfse.xsd">' +
                               '<' + Prefixo4 + 'InfDeclaracaoPrestacaoServico' +
                                 RetornarConteudoEntre(TNFSeGerarNFSe(Self).FNotasFiscais.Items[I].XML_Rps_Ass,
                                   '<' + Prefixo4 + 'InfDeclaracaoPrestacaoServico', '</Signature>') +
                               '</Signature>'+
...
begin
 ...
 //DONE: Criado por Thiago InterNews
 if (FProvedor = proPronim) then
   FNameSpaceDad := '';
 //DONE: Fim criado por Thiago InterNews
   ...
   for i := 0 to TNFSeGerarLoteRPS(Self).FNotasFiscais.Count-1 do
    begin
     case FProvedor of
      ...
      pro4R, proFiorilli, proProdata, proVitoria, proPVH, proAgili, proEReceita,
...
begin
 ...
 begin
   if not (TACBrNFSe( FACBrNFSe ).Configuracoes.WebServices.Provedor in [proDigifred, proProdata,
          ...
          proISSDSF, proFiorilli, proFreire, proTecnos, proDBSeller, proCTA,
          proEReceita]) then //adicionei o provedor DBSeller - Rodrigo Custódio
    begin
...
//      else
      ...
      begin
        Result := False;
        Msg := 'Erro desconhecido.' + #13#10 +
               'Retorno da URL '+ FURL + ':' + #13#10 +
               StrStream.DataString;
        if Assigned(TACBrNFSe( FACBrNFSe ).OnGerarLog) then
          TACBrNFSe( FACBrNFSe ).OnGerarLog(Msg);
...
//    else
  ...
  try
   ...
   if (FProvedor in [proEquiplano, proIssDsf, proEL, proCTA]) then
    ...
    if FProvedor in [proSisPMJP, proEReceita] then
      Prefixo3 := 'nfse:';
