...
type
 ...
 TNFSeW = class(TPersistent)
  private
    ...
    procedure GerarListaServicos_proTecnos;
...
//                         IntToStrZero(StrToIntDef(FNFSe.NumeroLote, 1), 16);
  ...
  proInfisc: FNFSe.InfID.ID := FNFSe.ChaveNFSe; // FNFSe.Numero;
...
begin
  Gerador.wGrupoNFSe('Servico');
   ...
   begin
     Gerador.wCampoNFSe(tcStr, '#30', 'CNAE', 01, 0007, 1, SomenteNumeros(NFSe.Servico.CodigoCnae), '');
   end;

   if not (FProvedor in [proPublica, proNFSeBrasil, proEReceita]) then
   begin
...
procedure TNFSeW.GerarListaServicos_proTecnos;
var
  i: Integer;
begin
  Gerador.wGrupoNFSe('Servico');
