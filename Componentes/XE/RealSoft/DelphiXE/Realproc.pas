...
begin
  ...
  ScanBlanks(S, Pos);
 // if FFormatoBR <> Nil  then
 //  FreeAndNil(FFormatoBR);
...
begin
  Result := False;
  FFormatoBR := TFormatSettings.Create;
  if not (ScanNumber(S, Pos, Hour) and ScanChar(S, Pos, FFormatoBR.TimeSeparator) and
    ScanNumber(S, Pos, Min)) then Exit;
  Sec := 0;
  if ScanChar(S, Pos, FFormatoBR.TimeSeparator) then
    if not ScanNumber(S, Pos, Sec) then Exit;
  BaseHour := -1;
  if ScanString(S, Pos, FFormatoBR.TimeAMString) or ScanString(S, Pos, 'AM') then
    BaseHour := 0
  else
    if ScanString(S, Pos, FFormatoBR.TimePMString) or ScanString(S, Pos, 'PM') then
      BaseHour := 12;
  if BaseHour >= 0 then
  begin
