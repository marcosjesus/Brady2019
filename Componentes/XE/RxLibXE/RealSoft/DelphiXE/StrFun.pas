...
begin
  ...
  try
    ...
    StrError := true;
    Result := '  ' + FormatSettings.DateSeparator + '  ' +FormatSettings.DateSeparator + '  ';
  end;
end;
...
Begin
    ...
    StrError := False ;
    S := Str_LRTrim(S) ;
    If ( Not GetTextExtentPoint32(hDC,PAnsiChar(S),Length(S),TagSize) ) Then
    Begin
       StrError := True ;
       Exit ;
    End ;
...
begin
   ...
   Begin
       S := ' ' + S ;
       (* Win32 *)
       If ( Not GetTextExtentPoint32(hDC,PAnsiChar(S),Length(S),TextSize) ) Then
       Begin
          StrError := True ;
          Exit ;
       End ;
...
Begin
   ...
   Begin
       ...
       (* Win32 *)
       If ( Not GetTextExtentPoint32(hDC,PAnsiChar(Aux),Length(Aux),TextSize) ) Then
       Begin
          StrError := True ;
          Exit ;
       End ;
