...
uses
  ...
  cxGrid, FireDAC.UI.Intf, FireDAC.Stan.Def, FireDAC.Stan.Pool, FireDAC.Phys;
...
begin
   ...
   edtCodigo.Text := sqlConsulta.FieldByName('codusuario').AsString;
...
begin
  ...
  begin
     ...
     Try
       sqlAux.Connection := DB_Conect.SQLConnection;
       sqlAux.Close;
       sqlAux.SQL.Clear;
       sqlAux.SQL.Add('Delete From Usuario where codusuario = :codusuario');
       sqlAux.Params.ParamByName('codusuario').AsString := sqlConsulta.FieldByName('codusuario').AsString;
       sqlAux.ExecSQL;
       sqlConsulta.Close;
       sqlConsulta.Open;
       PageControl1.ActivePage := TabConsulta;
...
begin
   ...
   Try
     ...
     sqlAux.Close;
     sqlAux.SQL.Clear;
     sqlAux.SQL.Add('Select Max(codusuario) + 1 as Registro From Usuario');
     sqlAux.Open;
...
Begin
   ...
   Try
     ...
     sqlAux.Close;
     sqlAux.SQL.Clear;
     sqlAux.SQL.Add('Insert Into usuario (Nome, Senha) Values (:Nome, :Senha)');
...
Begin
   ...
   Try
     ...
     sqlAux.Close;
     sqlAux.SQL.Clear;
     sqlAux.SQL.Add('Update Usuario Set Nome = :Nome, Senha = :Senha Where codusuario = :codusuario');

     sqlAux.ParamByName('Nome').AsString         := edtNome.Text;
     sqlAux.ParamByName('Senha').AsString        := EdtSenha.Text;
     sqlAux.ParamByName('codusuario').AsString  := edtCodigo.Text;
