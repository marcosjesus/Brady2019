...
Uses
     SysUtils, Windows, DB, DBTables, Classes, Forms, extctrls,
     ComCtrls, StdCtrls, Controls ;
...
Type
   ...
   Type TSQLTable = Class(TComponent)
   	   Private
           ...
           FTableName   : String[20] ;                         // -> Nome da tabela default
       ...
       Public
           ...
           Procedure ResetAllUpdFields ;
           Function SetUpdField ( FieldName : String ) : Boolean ;
           Function ResetUpdField ( FieldName : String ) : Boolean ;
...
Uses
   rsFlyovr, rsEdit, StrFun, TestFun, ObjFun, GN_RadioGroup ;
...
Begin
   If ( FKeyCount = 0 ) Then
   Begin
       SetError('SQLTable Error: Select -> Key Fields not found') ;
       Result := False ;
       Exit ;
   End ;
   Case FDBType Of
       ...
       DBTYPE_SQLSERVER_65 : Begin
             FSql := 'Select *, TS As TimeStamp From ' + FTableName + ' Where '
             For i:=1 To FKeyCount Do
             Begin
                f := FKeyFields[i] ;
...
Begin
   If ( FKeyCount = 0 ) Then
   Begin
       SetError('SQLTable Error: Update -> Key Fields not found') ;
       Result := False ;
       Exit ;
   End ;
   ...
   If ( FDBType = DBTYPE_SQLSERVER_65 ) Then
      FSql := FSql + ' TS = :Par_TS'
   Else If ( FDBType = DBTYPE_SQLSERVER_70 ) Then
...
Begin
   If ( FKeyCount = 0 ) Then
   Begin
		SetError('SQLTable Error: Delete -> Key Fields not found') ;
       Result := False ;
       Exit ;
   End ;
 ...
 * Parametros: FieldName -> Determinador do campo                            *
(* ------------------------------------------------------------------------- *)
Function TSqlTable.ResetUpdField ( FieldName : String ) : Boolean ;
Begin
   FFieldPos := GetFieldPos(FieldName) ;
   If ( FFieldPos <> -1 ) Then
   Begin
		FFieldName := FieldName ;
		FFieldDesc[FFieldPos].FUpdOk := False ;
		SetError ;
		Result := True ;
   End
   Else Begin
...
(* ------------------------------------------------------------------------- *)
Procedure TSqlTable.ResetAllUpdFields ;
Var
   i : Integer ;
Begin
   For i:=0 To FFieldCount-1 Do
		FFieldDesc[i].FUpdOk := False ;
End ;
