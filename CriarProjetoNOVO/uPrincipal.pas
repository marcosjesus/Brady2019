...
type
  TfrmPrincipal = class(TForm)
    ...
    dxnNotaFiscal: TdxNavBarItem;
    dxnAbcPorVenda: TdxNavBarItem;
    dxnAbcPorProduto: TdxNavBarItem;
    actImportarNF: TAction;
    actAbcPorVenda: TAction;
    dxnCadastro: TdxNavBarGroup;
    ...
    procedure actAbcPorVendaExecute(Sender: TObject);
    procedure actAbcPorProdutoExecute(Sender: TObject);
...
uses Global, Login,  DBConect,  uUsuario, Parametros,
      PerfilOpe, Perfil, Operacao,uFrmImportarNFeEntrada,CadEmpresa,uRelatorioManutencaoClientes, CadFornecedor;
...
begin
   ...
   if Credencias.Host = '' then
    begin
      Mens_MensInf('Problemas na Leitura do Arquivo ConexaoABC.Ini !');
   ...
   begin
      ...
      SQLConnection.Params.LoadFromFile( ExtractFilePath(Application.ExeName) + '\' +  'ConexaoABC.ini' );
      SQLConnection.Open;
   end;
 ...
 //   showmessage('arquivo ini' + varArquivoINI.Text);
   ...
   GDatabase   := 'DB_Loja';
   SQLTable_Start('filial',DBTYPE_SQLSERVER_70) ;
...
{-----------------------------------------------------------------------}
procedure TfrmPrincipal.actAbcPorProdutoExecute(Sender: TObject);
begin
      If ( Not Glob_GetAccess('CON_ABCPORPRODUTO','') ) Then
   Begin
       Mens_MensInf('Operação (CON_ABCPORPRODUTO) não liberada para o usuário !') ;
       Exit;
   End ;
end;

procedure TfrmPrincipal.actAbcPorVendaExecute(Sender: TObject);
begin

   If ( Not Glob_GetAccess('CON_ABCPORVENDA','') ) Then
   Begin
       Mens_MensInf('Operação (CON_ABCPORVENDA) não liberada para o usuário !') ;
       Exit;
   End ;

   Fr_RelatorioManutencaoClientes:=TFr_RelatorioManutencaoClientes.Create(Self);
   Fr_RelatorioManutencaoClientes.Show;
...
begin
   ...
   Begin
       Mens_MensInf('Operação (CAD_EMPRESA) não liberada para o usuário !') ;
       Exit;
   End ;
...
begin
   ...
   Begin
       Mens_MensInf('Operação (CAD_FORNECEDOR) não liberada para o usuário !') ;
       Exit;
   End ;
...
procedure TfrmPrincipal.actImportarNFExecute(Sender: TObject);
begin


   If ( Not Glob_GetAccess('IMP_NOTAFISCAL','') ) Then
   Begin
       Mens_MensInf('Operação (IMP_NOTAFISCAL) não liberada para o usuário !') ;
       Exit;
   End ;

  frmImportarNFeEntrada := TfrmImportarNFeEntrada.Create(Self) ;
  frmImportarNFeEntrada.Show ;
...
//   qryAux.SQL.Add('Select Convert(DateTime, (convert(varchar(10), getdate(), 103))) as DataServer ');
   DB_Conect.sqlAuxiliar.SQL.Add('Select Now() as DataServer ');
   ...
   dxnCadastro.Visible     := ( Glob_GetAccess('MENU-CAD','') ) ;
   dxnImportar.Visible     := ( Glob_GetAccess('MENU-IMP','') ) ;
